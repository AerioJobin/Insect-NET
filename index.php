...
:root {
    ...
    --shadow:rgba(0,0,0,0.07);--shadow-md:rgba(0,0,0,0.13);
    --on-bg:#dcfce7;--on-fg:#16a34a;
    --st-bg:#fef9c3;--st-fg:#92400e;
    --off-bg:#fee2e2;--off-fg:#ef4444;
    --radius:12px;--tr:0.22s ease;
}
[data-theme="dark"] {
    --bg:#0e0c11;--surface:#19161f;--surface2:#231f2b;
    --border:#38334a;--text:#e8e0ec;--text-dim:#9a8fa8;
    --shadow:rgba(0,0,0,0.45);--shadow-md:rgba(0,0,0,0.65);
    --on-bg:#052e16;--on-fg:#4ade80;
    --st-bg:#1c1500;--st-fg:#fbbf24;
    --off-bg:#1f0808;--off-fg:#f87171;
}
*,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
body{font-family:'Inter',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;line-height:1.6;transition:background var(--tr),color var(--tr);}

/* ═══ THEME TOGGLE ═══ */
.theme-toggle{position:fixed;top:16px;right:16px;z-index:900;width:38px;height:38px;border-radius:50%;border:1px solid var(--border);background:var(--surface);cursor:pointer;font-size:1.05em;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 12px var(--shadow);transition:all var(--tr);}
.theme-toggle:hover{background:var(--surface2);transform:scale(1.1);}

/* ═══ TOASTS ═══ */
#toast-container{position:fixed;top:20px;right:64px;z-index:9999;display:flex;flex-direction:column;gap:8px;pointer-events:none;}
.toast{background:#1a1425;color:#ede8f5;padding:11px 16px;border-radius:10px;font-size:0.82em;display:flex;align-items:center;gap:9px;box-shadow:0 4px 20px rgba(0,0,0,0.4);animation:toastIn 0.3s ease forwards;pointer-events:all;max-width:280px;border-left:3px solid var(--accent);}
...
/* ═══ HEADER ═══ */
.header{background:linear-gradient(135deg,var(--surface) 0%,var(--surface2) 100%);border:1px solid var(--border);padding:24px 28px;border-radius:16px;margin-bottom:24px;box-shadow:0 8px 32px var(--shadow);}
...
.logo-divider{width:1px;height:32px;background:linear-gradient(180deg,transparent,var(--border),transparent);}
.header h1{font-family:'Space Mono',monospace;font-size:clamp(1.3em,3.5vw,2em);color:var(--accent);letter-spacing:4px;}
.header-subtitle{font-size:0.72em;color:var(--text-dim);letter-spacing:2px;margin-top:2px;}

/* Refresh indicator */
.refresh-indicator{display:flex;align-items:center;gap:8px;font-size:0.78em;color:var(--text-dim);}
.refresh-dot{width:8px;height:8px;border-radius:50%;background:var(--on-fg);flex-shrink:0;}
.refresh-dot.pulsing{animation:rdPulse 0.7s ease;}
@keyframes rdPulse{0%{box-shadow:0 0 0 0 rgba(74,222,128,0.7)}70%{box-shadow:0 0 0 8px rgba(74,222,128,0)}100%{box-shadow:0 0 0 0 rgba(74,222,128,0)}}
.refresh-ring{width:26px;height:26px;}
.refresh-ring svg{transform:rotate(-90deg);display:block;}
.refresh-ring circle{fill:none;stroke:var(--accent);stroke-width:2.5;stroke-dasharray:69;stroke-dashoffset:0;stroke-linecap:round;transition:stroke-dashoffset 1s linear;}

/* ═══ DEVICE CARDS ═══ */
.device-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:18px;margin-bottom:24px;}
.device-card{background:var(--surface);border:1px solid var(--border);border-left:4px solid var(--border);padding:18px;border-radius:var(--radius);cursor:pointer;transition:transform var(--tr),box-shadow var(--tr),border-color var(--tr);}
.device-card:hover{transform:translateY(-4px);box-shadow:0 12px 32px var(--shadow-md);}
.device-card.status-online{border-left-color:var(--on-fg);}
.device-card.status-stale{border-left-color:var(--st-fg);}
.device-card.status-offline{border-left-color:var(--off-fg);}
.device-card-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;}
.device-icon{width:34px;height:34px;border-radius:8px;background:linear-gradient(135deg,var(--primary),var(--accent));display:flex;align-items:center;justify-content:center;font-size:0.95em;flex-shrink:0;}
.device-card h3{font-size:0.95em;font-weight:600;margin:2px 0;}
.device-meta{font-size:0.8em;color:var(--text-dim);margin-top:3px;}
.device-status{display:inline-flex;align-items:center;gap:5px;padding:2px 9px;border-radius:20px;font-size:0.7em;font-weight:700;}
.device-status::before{content:'●';font-size:0.8em;}
.online{background:var(--on-bg);color:var(--on-fg);}
.stale{background:var(--st-bg);color:var(--st-fg);}
.offline{background:var(--off-bg);color:var(--off-fg);}
.device-battery-bar{margin-top:11px;height:4px;background:var(--surface2);border-radius:4px;overflow:hidden;}
.device-battery-fill{height:100%;width:0%;border-radius:4px;background:linear-gradient(90deg,var(--primary),var(--accent));transition:width 0.5s ease;}

/* ═══ MAP SECTION ═══ */
.map-section{background:var(--surface);border:1px solid var(--border);border-radius:16px;overflow:hidden;box-shadow:0 8px 32px var(--shadow);}
.map-header{display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid var(--border);}
.map-header h3{font-size:0.85em;font-weight:700;letter-spacing:1.5px;}
.last-updated{font-size:0.74em;color:var(--text-dim);}
.map-expand-btn{background:var(--surface2);border:1px solid var(--border);border-radius:6px;padding:4px 10px;font-size:0.72em;cursor:pointer;color:var(--text-dim);transition:all var(--tr);}
.map-expand-btn:hover{background:var(--border);color:var(--text);}
#map{height:320px;}
#mapFullscreen{position:fixed;inset:0;z-index:5000;display:none;}
#mapFullscreen.open{display:block;}
#mapFsClose{position:absolute;top:14px;right:14px;z-index:5001;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 14px;font-size:0.82em;cursor:pointer;box-shadow:0 4px 16px var(--shadow);}
#mapFs{width:100%;height:100vh;}

/* ═══ DEVICE DETAIL ═══ */
.back-btn{display:inline-flex;align-items:center;gap:6px;margin-bottom:18px;text-decoration:none;color:var(--primary);font-weight:600;font-size:0.88em;transition:gap var(--tr);}
.back-btn:hover{gap:10px;}
.panel-row{display:grid;grid-template-columns:minmax(0,2fr) minmax(250px,1fr);gap:16px;margin-bottom:12px;}
@media(max-width:768px){.panel-row{grid-template-columns:1fr;}}
.status-panel{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:18px;display:flex;flex-direction:column;gap:14px;box-shadow:0 4px 16px var(--shadow);}
.status-row{display:flex;align-items:center;gap:10px;width:100%;}
.status-label{font-family:'Space Mono',monospace;letter-spacing:1px;font-size:0.74em;color:var(--text-dim);text-transform:uppercase;min-width:70px;}
.status-track{flex:1;height:8px;background:var(--surface2);border-radius:999px;overflow:hidden;}
.status-fill{height:100%;width:0%;background:linear-gradient(90deg,var(--primary),var(--accent));transition:width 0.5s ease;}
.status-text{font-size:0.78em;color:var(--text);font-weight:600;min-width:86px;text-align:right;}

/* ═══ GALLERY ═══ */
.gallery-controls{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:10px;}
.gallery-controls select{padding:7px 11px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:0.82em;cursor:pointer;}
.gallery-controls select:focus{outline:none;border-color:var(--accent);}
.image-gallery{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px;}
.image-container{background:var(--surface);border:1px solid var(--border);padding:9px;border-radius:var(--radius);position:relative;transition:border-color var(--tr),box-shadow var(--tr);}
.image-container:hover{border-color:var(--accent);box-shadow:0 4px 20px var(--shadow-md);}
.img-wrapper{position:relative;width:100%;padding-bottom:75%;background:var(--surface2);border-radius:8px;overflow:hidden;margin-bottom:7px;}
.img-wrapper img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;border-radius:8px;cursor:pointer;opacity:0;transition:opacity 0.3s ease;}
.img-wrapper img.loaded{opacity:1;}
.img-wrapper::before{content:'';position:absolute;inset:0;background:linear-gradient(90deg,var(--surface2) 25%,var(--border) 50%,var(--surface2) 75%);background-size:200% 100%;animation:shimmer 1.4s infinite;border-radius:8px;transition:opacity 0.3s;}
.img-wrapper.img-ready::before{opacity:0;pointer-events:none;}
@keyframes shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
.delete-btn{position:absolute;top:7px;right:7px;border:none;border-radius:6px;padding:3px 8px;cursor:pointer;font-size:0.7em;font-weight:700;z-index:10;opacity:0;transition:opacity 0.2s;white-space:nowrap;background:rgba(220,38,38,0.85);color:#fff;}
.image-container:hover .delete-btn{opacity:1;}
.delete-btn.confirm-mode{background:#7f1d1d;}
.image-container.deleting{opacity:0.4;pointer-events:none;transition:opacity 0.3s;}
.img-filename{font-size:0.7em;color:var(--text-dim);word-break:break-all;}
.date-heading{grid-column:1/-1;font-family:'Space Mono',monospace;padding:7px 0;border-bottom:1px dashed var(--border);color:var(--primary);font-weight:700;font-size:0.82em;margin-top:14px;}
...
/* ═══ LIGHTBOX ═══ */
.lightbox{position:fixed;inset:0;background:rgba(0,0,0,0.92);display:none;align-items:center;justify-content:center;z-index:1000;padding:16px;}
.lightbox.open{display:flex;}
.lightbox-panel{position:relative;background:#0f0f0f;border-radius:12px;padding:8px;max-width:min(94vw,900px);}
.lightbox-panel img{max-width:100%;max-height:74vh;border-radius:8px;display:block;}
.lightbox-close{position:absolute;top:-13px;right:-13px;width:30px;height:30px;border-radius:50%;border:2px solid rgba(255,255,255,0.25);background:#1a1a1a;color:#fff;font-size:0.9em;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:10;transition:background 0.2s,border-color 0.2s;}
.lightbox-close:hover{background:#ef4444;border-color:#ef4444;}
.lightbox-nav-bar{display:flex;align-items:center;justify-content:space-between;padding:9px 4px 4px;}
.lightbox-nav{border:none;background:rgba(255,255,255,0.13);color:#fff;padding:5px 14px;border-radius:8px;cursor:pointer;font-size:0.9em;transition:background 0.2s;}
.lightbox-nav:hover{background:rgba(255,255,255,0.22);}
#lightboxCap{color:rgba(255,255,255,0.6);font-size:0.76em;text-align:center;flex:1;padding:0 10px;}

/* ═══ LANDING ═══ */
.landing{height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;position:relative;overflow:hidden;}
.landing-grid{position:absolute;inset:0;background-image:linear-gradient(var(--border) 1px,transparent 1px),linear-gradient(90deg,var(--border) 1px,transparent 1px);background-size:44px 44px;opacity:0.35;pointer-events:none;}
.landing-orb{position:absolute;border-radius:50%;filter:blur(90px);pointer-events:none;}
.orb1{width:480px;height:480px;background:rgba(138,34,69,0.18);top:-120px;left:-120px;animation:floatA 9s ease-in-out infinite;}
.orb2{width:380px;height:380px;background:rgba(196,69,105,0.12);bottom:-80px;right:-80px;animation:floatA 11s ease-in-out infinite reverse;}
@keyframes floatA{0%,100%{transform:translate(0,0)}50%{transform:translate(28px,18px)}}
.brand-card{display:flex;align-items:center;gap:20px;padding:18px 26px;background:var(--surface);border:1px solid var(--border);border-radius:16px;box-shadow:0 10px 40px var(--shadow);margin-bottom:32px;position:relative;z-index:1;}
.brand-divider{width:1px;align-self:stretch;background:linear-gradient(180deg,transparent,var(--border),transparent);}
.landing h1{font-family:'Space Mono',monospace;font-size:clamp(2em,6vw,4.2em);color:var(--primary);letter-spacing:8px;position:relative;z-index:1;}
.landing-sub{letter-spacing:3px;margin-bottom:36px;color:var(--text-dim);font-size:clamp(0.72em,2vw,0.9em);position:relative;z-index:1;}
.cta-btn{background:linear-gradient(135deg,var(--primary),var(--accent));color:#fff;padding:14px 44px;border-radius:30px;text-decoration:none;font-weight:700;letter-spacing:1.5px;font-size:0.88em;position:relative;z-index:1;box-shadow:0 6px 24px rgba(138,34,69,0.35);transition:transform var(--tr),box-shadow var(--tr);}
.cta-btn:hover{transform:translateY(-2px);box-shadow:0 10px 30px rgba(138,34,69,0.45);}

/* ═══ MISC ═══ */
.last-updated-row{display:flex;justify-content:flex-end;margin:-2px 0 14px;}
...
<div class="landing">
    ...
    <div class="brand-card">
        <img src="assets/neuronics_logo.png" style="height:76px;" onerror="this.style.display='none'" alt="NeuRonICS">
        <div class="brand-divider"></div>
        <img src="assets/iisc_logo.jpg" style="height:120px;" onerror="this.style.display='none'" alt="IISc">
    </div>
    <h1>INSECT NET</h1>
        ...
        <div class="header-content">
            <div class="header-left">
                <div class="header-logos">
                    <img src="assets/neuronics_logo.png" onerror="this.style.display='none'" alt="NeuRonICS">
                    <div class="logo-divider"></div>
                    <img src="assets/iisc_logo.jpg" onerror="this.style.display='none'" alt="IISc">
                </div>
                <div>
                    <h1>INSECT NET</h1>
    ...
    <div class="panel-row">
        ...
        <div class="status-panel">
            <div class="status-row">
                ...
                <div class="status-track"><div id="waveshareStatusFill" class="status-fill"></div></div>
                <div id="waveshareStatusText" class="status-text">Awaiting&hellip;</div>
            </div>
...
function showAt(i) { if (!allImages.length) return; curIdx = (i + allImages.length) % allImages.length; lbImg.src = allImages[curIdx].src; lbCap.textContent = `${allImages[curIdx].name}  (${curIdx+1} / ${allImages.length})`; }
...
function fetchStatus() {
    ...
    .then(r => r.json()).then(data => {
        ...
        const upEl = document.getElementById('deviceLastUpdated');
        if (upEl) upEl.textContent = `Last updated: ${new Date().toLocaleTimeString()} (fetch failed)`;
    });
}
